...
enum_start (gegl_zwind_style)
  enum_value (GEGL_ZWIND_STYLE_ZWIND, "zwind", N_("zWind"))
  enum_value (GEGL_ZWIND_STYLE_BLAST, "blast", N_("Blast"))
enum_end (GeglzWindStyle)

enum_start (gegl_zwind_direction)
  enum_value (GEGL_ZWIND_DIRECTION_LEFT, "left", N_("Left"))
  enum_value (GEGL_ZWIND_DIRECTION_RIGHT, "right", N_("Right"))
  enum_value (GEGL_ZWIND_DIRECTION_TOP, "top", N_("Top"))
  enum_value (GEGL_ZWIND_DIRECTION_BOTTOM, "bottom", N_("Bottom"))
enum_end (GeglzWindDirection)

enum_start (gegl_zwind_edge)
  enum_value (GEGL_ZWIND_EDGE_BOTH, "both", N_("Both"))
  enum_value (GEGL_ZWIND_EDGE_LEADING, "leading", N_("Leading"))
  enum_value (GEGL_ZWIND_EDGE_TRAILING, "trailing", N_("Trailing"))
enum_end (GeglzWindEdge)

property_enum (style, _("Style"),
               GeglzWindStyle, gegl_zwind_style,
               GEGL_ZWIND_STYLE_ZWIND)
  description (_("Style of effect"))

property_enum (direction, _("Direction"),
               GeglzWindDirection, gegl_zwind_direction,
               GEGL_ZWIND_DIRECTION_LEFT)
  description (_("Direction of the effect"))

property_enum (edge, _("Edge Affected"),
               GeglzWindEdge, gegl_zwind_edge,
               GEGL_ZWIND_EDGE_LEADING)
...
#define GEGL_OP_C_SOURCE zzwind.c
#define GEGL_OP_NAME     zzwind
                ...
                gfloat       *derivative)
{
  ...
  if (edge == GEGL_ZWIND_EDGE_BOTH)
    {
      for (i = 0; i < 4; i++)
        derivative[i] = fabs (derivative[i]);
    }
  else if (edge == GEGL_ZWIND_EDGE_LEADING)
    {
      for (i = 0; i < 4; i++)
...
static void
render_zwind_row (gfloat         *buffer,
                 gint            n_components,
                 gint            lpi,
                 GeglProperties *o,
                 gint            x,
                   ...
                   const GeglRectangle *roi)
{
  ...
  if (in_rect && ! gegl_rectangle_is_infinite_plane (in_rect))
    {
      if (o->direction == GEGL_ZWIND_DIRECTION_LEFT ||
          o->direction == GEGL_ZWIND_DIRECTION_RIGHT)
        {
          result.x     = in_rect->x;
                         ...
                         const GeglRectangle *roi)
{
  ...
  if (in_rect && ! gegl_rectangle_is_infinite_plane (in_rect))
    {
      if (o->direction == GEGL_ZWIND_DIRECTION_TOP)
        {
          result.height = in_rect->height - roi->y;
        }
      else if (o->direction == GEGL_ZWIND_DIRECTION_BOTTOM)
        {
          result.y      = in_rect->y;
          result.height = in_rect->height - roi->y + roi->height;
        }
      else if (o->direction == GEGL_ZWIND_DIRECTION_RIGHT)
        {
          result.x     = in_rect->x;
         ...
         gint                 level)
{
  ...
  horizontal_effect = (o->direction == GEGL_ZWIND_DIRECTION_LEFT ||
                       o->direction == GEGL_ZWIND_DIRECTION_RIGHT);

  need_reverse = (o->direction == GEGL_ZWIND_DIRECTION_LEFT ||
                  o->direction == GEGL_ZWIND_DIRECTION_TOP);
  ...
  for (y = row_start; y < row_end; y++)
    {
      ...
      if (o->style == GEGL_ZWIND_STYLE_ZWIND)
        {
          last_pix  = row_size - (n_components * COMPARE_WIDTH);
          skip_rows = FALSE;
          render_zwind_row (row_buf, n_components, last_pix, o, row_rect.x, y);
        }
      else
        {
...
gegl_op_class_init (GeglOpClass *klass)
{
  ...
  gegl_operation_class_set_keys (operation_class,
     ...
     "categories",     "hidden",
     "license",        "GPL3+",
     "reference-hash", "0991d4418147d2c155012ce1d522f6e",
     "description",    _("zWind-like bleed effect"),
     NULL);
}
